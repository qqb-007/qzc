...
(function () {
	...
	Smart.widgetExtend({
		...
		onPrepare: function () {
			this.cache = {};
			var scripts = [];
			var script = this.S.node.attr('s-show');
			scripts.push("(function(){");
			scripts.push("      return function(){");
			// scripts.push("      debugger");
			scripts.push('return ' + script);
			scripts.push("      }");
			scripts.push("})();//# sourceURL=show_" + Smart.hashCode(script) + "_template.js");
			var script = scripts.join("\n");
			var fn;
			var fn_map = this.S.contextValue("s-show-fn_map");
			if (!fn_map) {
				fn_map = {};
		...
		dataSetter: function (data) {
			var fn = this.widget.show.cache[TABLE_FN_KEY];
			var flag = fn.call(data);
			if (!flag) {
				this.node.hide();
			} else {
				this.node.show();
